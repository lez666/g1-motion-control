...
class IsaacSim(BaseSimulator):
    ...
    def _load_scene_config(self) -> None:
        ...
        if scene_config.scene_files is not None and len(scene_config.scene_files) > 0:
    ...
    def _load_scene_files(self, scene_config: SceneConfig) -> None:
        ...
        """
        if scene_config.scene_files is None or len(scene_config.scene_files) == 0:  # None or empty list
            # This should not happen if check at line 498 is correct, but handle it anyway
            return  # Skip loading if None or empty
